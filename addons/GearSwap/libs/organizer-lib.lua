...
function org.export_set()
    ...
    local ward,inv = T{},T{}
    for itemIndex,item in ipairs(flattab) do
        if org.possible_to_store_in_wardrobe(item.id) then
            ward:append(item)
        else
            inv:append(item)
        end
    ...
    if #ward > wardrobe_max then
        -- if we've put more into the wardrobe than what is possible, we need to move the excess elements into the inventory
        inv:extend(ward:slice(wardrobe_max + 1))
        
        for ward_index_to_remove = #ward, wardrobe_max + 1, -1 do
            table.remove(ward, ward_index_to_remove)
        end
    end
    
    if #inv > inventory_max then
        windower.add_to_chat(123,'Organizer Library: Your sets and organizer_items tables contain too many items.')
    end
...
function org.possible_to_store_in_wardrobe(id)
    return gearswap.res.items[id] and (gearswap.res.items[id].type == 4 or gearswap.res.items[id].type == 5)
end
