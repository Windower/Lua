...
function validate_inventory(filter)
    gs_add_to_chat(123,'Checking for items in inventory that are not used in your gear sets.')
    ...
    display_list:map(function(item) windower.add_to_chat(1, string.char(0x1F,120)..(string.gsub(item, "^%l", string.upper))..string.char(0x1E,0x01)) end)
    gs_add_to_chat(123,'Final count = '..tostring(display_list:length()))
end
...
function validate_sets(filter)
    gs_add_to_chat(123,'Checking for items in gear sets that are not in your inventory.')
    ...
    for i,v in pairs(gear_table) do
        ...
        if type(name) == 'string' and name ~= 'empty' and name ~= '' and type(i) == 'string'then
            if not slot_map[i] then
                gs_add_to_chat(123,windower.to_shift_jis(tostring(i))..' contains a "name" element but is not a valid slot.')
